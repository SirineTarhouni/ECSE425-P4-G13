...
architecture behavioral of alu is
  ...
  -- ALU operation codes, its the values control unit will produce
  ...
  -- signals to be used later for computation
  ...
  signal shamt  : natural range 0 to 31; -- shift amount (becase we only use the low 5 bits of op_b)
  signal mul_result : signed(63 downto 0); -- for MUL, we need a 64-bit buffer to hold the full product
  ...
  process(alu_op, op_a, op_b, a_signed, b_signed, a_unsigned, b_unsigned, shamt, mul_result)
    begin
      case alu_op is 
        ...
        -- SUB (branches do NOT use the ALU for their comparison)
        when ALU_SUB =>
          result <= std_logic_vector(a_signed - b_signed);
        ...
        -- SLL: shift left (filling with 0s)
        when ALU_SLL =>
          result <= std_logic_vector(shift_left(a_unsigned, shamt));
        -- SRL: shift right, filling with 0s
        when ALU_SRL =>
          result <= std_logic_vector(shift_right(a_unsigned, shamt));

        -- SRA: Shift Right Arithmetic, replicates MSB
        when ALU_SRA =>
          result <= std_logic_vector(shift_right(a_signed, shamt));

        -- SLT: Set Less Than (signed)-> outputs 1 if op_a < op_b as signed integers, else 0
        when ALU_SLT =>
        ...
        -- LUI: load upper immediate(already in op_b)
        when ALU_LUI =>
          result <= op_b;

        -- MUL, produce a 64-bit product, we only keep the lower
        when ALU_MUL =>
          mul_result  <= a_signed * b_signed;
          result  <= std_logic_vector(mul_result(31 downto 0));

        -- Default outputs zero for any undefined opcode
        when others =>
          result <= (others => '0');
