...
--
-- Testbench for imm_gen.vhd.
--
-- Strategy: for each RISC-V immediate format, one or more
-- real encoded instructions are applied and the output is
-- compared against the hand-computed expected value.  Any
-- mismatch prints a FAIL message and sets a flag; a single
-- PASS / FAIL summary is printed at the end.
--
-- Formats exercised:
--   I-type ALU  : addi x1, x2, -1        (sign-extend negative)
--                 addi x0, x0, 0         (zero / NOP)
--                 slli x3, x3, 4         (shift: only imm[4:0] matters)
--                 sltiu x4, x5, 2048     (large positive)
--   I-type Load : lw   x6, 100(x7)       (positive offset)
--                 lb   x8, -4(x9)        (negative offset)
--                 lbu  x10, 8(x11)       (zero-extend DATA, but imm sign-extends)
--   I-type JALR : jalr x0, x1, -8        (negative)
--   S-type      : sw   x2, 12(x3)        (positive, split bits)
--                 sb   x4, -1(x5)        (negative, split bits)
--   B-type      : beq  x1, x2, +8        (forward, small)
--                 bne  x3, x4, -16       (backward, negative)
--   U-type LUI  : lui  x5, 0xABCDE       (upper 20-bit pattern)
--   U-type AUIPC: auipc x6, 1            (minimal non-zero upper)
--   J-type JAL  : jal  x1, +4            (forward minimal)
--                 jal  x0, -4            (backward, negative)
--   R-type      : add  x1, x2, x3        (no immediate -> 0)
-- ============================================================
...
architecture behavioral of imm_gen_tb is
    ...
    -- --------------------------------------------------------
    -- Helper: apply one test case, compare, report.
    --
    -- We use a procedure so every check is one line at the
    -- call site, keeping the stimulus section readable.
    -- --------------------------------------------------------
    -- (procedures cannot call std.textio directly in all
    --  simulators without a package; we use report/assert)

    -- Track overall pass/fail across all cases.
    shared variable all_pass : boolean := true;

    -- Small delay between stimulus changes so combinational
    -- output settles before we sample it.
...
begin
    ...
    stimulus : process
        ...
        -- ----------------------------------------------------
        -- check_imm: apply an instruction, wait for
        -- combinational logic to settle, then assert that
        -- imm_out equals expected.
        -- ----------------------------------------------------
        procedure check_imm (
            test_name   : in string;
        ...
        -- Encoding helpers.
        --
        -- Each function constructs the 32-bit machine word for
        -- a specific instruction used in the tests below.
        -- Bit positions follow the RISC-V spec exactly.
        -- ----------------------------------------------------

        -- I-type: [31:20]=imm[11:0], [19:15]=rs1, [14:12]=funct3,
        --         [11:7]=rd, [6:0]=opcode
        function encode_I (
        ...
        -- S-type: [31:25]=imm[11:5], [24:20]=rs2, [19:15]=rs1,
        ...
        -- B-type: [31]=imm[12], [30:25]=imm[10:5], [24:20]=rs2,
        --         [19:15]=rs1, [14:12]=funct3, [11:8]=imm[4:1],
        --         [7]=imm[11], [6:0]=opcode
        -- imm is a 13-bit value (imm[0] is always 0).
        function encode_B (
            imm    : std_logic_vector(12 downto 0);  -- imm[12:0], imm[0] ignored
            rs2    : std_logic_vector(4  downto 0);
            rs1    : std_logic_vector(4  downto 0);
            funct3 : std_logic_vector(2  downto 0);
            opcode : std_logic_vector(6  downto 0)
        ...
        function encode_U (
            imm    : std_logic_vector(19 downto 0);  -- upper 20 bits
            rd     : std_logic_vector(4  downto 0);
            opcode : std_logic_vector(6  downto 0)
        ) return std_logic_vector is
        begin
        ...
        -- J-type: [31]=imm[20], [30:21]=imm[10:1], [20]=imm[11],
        --         [19:12]=imm[19:12], [11:7]=rd, [6:0]=opcode
        -- imm is a 21-bit value (imm[0] is always 0).
        function encode_J (
            imm    : std_logic_vector(20 downto 0);  -- imm[20:0], imm[0] ignored
            rd     : std_logic_vector(4  downto 0);
            opcode : std_logic_vector(6  downto 0)
        ) return std_logic_vector is
        begin
        ...
        -- R-type: [31:25]=funct7, [24:20]=rs2, [19:15]=rs1,
    ...
    begin
        ...
        -- slli x3, x3, 4
        -- For shift instructions funct7 occupies imm[11:5];
        -- the immediate generator still outputs the full sign-
        -- extended 12-bit field (imm = 0x004 = 0b000000000100).
        -- Only the low 5 bits [4:0] are the shift amount; the
        -- ALU extracts them.  The generator output is 0x00000004.
        check_imm(
        ...
        -- sltiu x4, x5, 2047   (largest positive 12-bit value)
        -- imm = 0x7FF = 2047
        -- expected: 0x000007FF
        check_imm(
        ...
        -- lbu x10, 8(x11)
        -- Immediate is still sign-extended (positive here -> same result).
        -- imm = 8 = 0x008
        -- expected: 0x00000008
        check_imm(
        ...
        -- sw x1, -2048(x2)   (most negative 12-bit value)
        -- imm = -2048 = 0x800
        -- expected: 0xFFFFF800
        check_imm(
        ...
        -- beq x1, x2, +8
        -- Branch offset = +8 bytes.  In B-type encoding,
        -- the 13-bit value is 0b0_000000_0100_0 = 0x0008
        -- but imm[0] is always 0, so we supply 13 bits where
        -- [0] is 0: imm = "0000000001000" (decimal 8).
        -- expected: 0x00000008
        check_imm(
        ...
        -- auipc x7, 0xFFFFF  (all-ones upper)
        -- expected: 0xFFFFF000
        check_imm(
        ...
        -- 0x1FFFFC = "1_11111111111_1_11111100"
        ...
        -- jal x1, +1048572  (largest positive JAL offset that fits,
        --                    2^20 - 4 = 1048572)
